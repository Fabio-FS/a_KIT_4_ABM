...
def calc_TA(graph,p, max_awareness = None):
    ...
    if susc_nodes == 0:
        ...
        for i,vertex in enumerate(graph.vs):
            if vertex["health_status"] == 1:
                if max_awareness == None:
                    TotAwa += (p**vertex["awareness"])/susc_nodes
                else:
                    TotAwa += ((p**vertex["awareness"])/susc_nodes   -   (p**max_awareness)/susc_nodes) * (1/ (1 - (p**max_awareness)/susc_nodes))
                beta_susc += vertex["beta"]/susc_nodes
    return TotAwa, beta_susc

def update_Funk_BC(G,rule,global_var):
    ...
    rr_3 = np.random.uniform(low = 0, high=1, size = g_b.vcount()) < alpha    #awareness spread, random number for each edge
    ...
    for i,vertex in enumerate(g_b.vs):
        #BC-averaging with one random neighbor
        awa_rnd_nghbr = g_b.vs[np.random.choice(g_b.neighbors(i))]["awareness"]
        influence = (vertex["awareness"] > awa_rnd_nghbr + 1)
        
        vertex["next_awareness"] = vertex["next_awareness"] + influence * rr_3[i] * (-vertex["next_awareness"] + awa_rnd_nghbr + 1)

    #decay: aware agents lose awareness          (behavior model)
    g_b.vs["next_awareness"] += (rr_2)*(np.array(g_b.vs["next_awareness"]) < max_awareness)

    #calculate new beta
    g_b.vs["next_beta"] =    (1-max_behavior)*beta0 + max_behavior* (beta0 * (( 1 - p ** np.array(g_b.vs["next_awareness"])) / ( 1 - p ** np.array(max_awareness) )) )
    #calculate the total awareness defined in Funk 2009
    TotAwa, PB_susc = calc_TA(g_h,p,max_awareness)
...
def init_Funk_BC(P_dyn, G,global_var):
    ...
    #set_initial_condition is not needed. Every agent starts with the same personal beta of beta0
    ...
    if P_dyn["BEHAVIOR"]["init"] == "rnd":
        G[bl].vs["awareness"] = np.random.uniform(low = 0,high = max_awareness, size = G[bl].vcount())
    elif P_dyn["BEHAVIOR"]["init"] == "max":
        G[bl].vs["awareness"] = np.full( shape=len(G[bl].vs), fill_value = max_awareness)
    G[bl].vs["next_awareness"] = G[bl].vs["awareness"]
    #initializing awareness as a high value (e.g. 99)
    #awareness goes from 0 to max_awareness. beta is calculated differently than in paper: max_awareness -> max_beta
...
def init_model(update_fct_dict, init_fct_dict):
    update_fct_dict["Funk_BC"] = update_Funk_BC
    init_fct_dict["Funk_BC"] = init_Funk_BC
