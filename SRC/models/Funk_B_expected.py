...
def update_Funk_B_expected(G,rule,global_var):
    """this simulates an SIR model, almost the complete model from Funk 2009 ( 2 diversions)
    1. health and behavior update are separated, first one then the other
    2. for each event, a node is assigned its expected awareness instead of one of the possibilities
    ...
    sender = [np.random.choice(g_b.neighbors(i)) for i in focal_agent]   #agents whose neighbors have few contacts send their awareness more often
...
def init_Funk_B_expected(P_dyn, G,global_var):
...
def init_model(update_fct_dict, init_fct_dict):
    update_fct_dict["Funk_B-expected"] = update_Funk_B_expected
    init_fct_dict["Funk_B-expected"] = init_Funk_B_expected
