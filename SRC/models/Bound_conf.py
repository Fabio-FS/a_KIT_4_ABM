    ...
    if susc_nodes == 0:
        ...
        for i,vertex in enumerate(graph.vs):
            if vertex["health_status"] == 1:
                #print(vertex["awareness"], p**vertex["awareness"])
                #print(((p**vertex["awareness"])   -   (p**max_awareness)) * (1/ (1 - (p**max_awareness))))
                #input()
...
def update_bounded_confidence(G,rule,global_var):
    ...
    for i,vertex in enumerate(g_b.vs):
        ...
        influence = (abs(vertex["awareness"]-awa_rnd_nghbr)<eps)
        
        vertex["next_awareness"] = ((1-mu) * vertex["next_awareness"] + mu * awa_rnd_nghbr) * influence + (1-influence)*vertex["next_awareness"]

    #decay: aware agents lose awareness          (behavior model)
    g_b.vs["next_awareness"] += (rr_2)*(np.array(g_b.vs["next_awareness"]) < max_awareness)

    #calculate new beta
    g_b.vs["next_beta"] =    (1-max_behavior)*beta0 + max_behavior* ( (1 - p ** np.array(g_b.vs["next_awareness"]) / (1 - p **max_awareness)) * beta0)
...
def init_bounded_confidence(P_dyn, G,global_var):
    ...
    rule  = {
        ...
        "eps" :P_dyn["BEHAVIOR"]["eps"]
    }
    return rule

def init_model(update_fct_dict, init_fct_dict):
    update_fct_dict["BC"] = update_bounded_confidence
    init_fct_dict["BC"] = init_bounded_confidence
